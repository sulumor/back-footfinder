...
test("route GET /scout/2", async () => {
  ...
  expect(res.body).toHaveProperty("role");
  expect(typeof res.body.role).toBe("string");
  expect(res.body.role).toMatch("recruteur");
  ...
  res.body.players.forEach((player) => {
    ...
    expect(player).toHaveProperty("genre");
    expect(typeof player.genre).toBe("string");
    expect(player.genre).toMatch(/^Homme|Femme$/);
    ...
    expect(typeof player.strong_foot).toBe("string");
    expect(player.strong_foot).toMatch(/^Droit|Gauche$/);
    ...
    expect(player.role).toMatch("joueur");
  });
});
